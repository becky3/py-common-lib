...
| 最低リクエスト間隔 | ハードリミット | 0.5 秒 | 引き下げ不可（引き上げ可、上限 60 秒） |
...
| 操作 | 振る舞い |
|------|---------|
| 初期化 | `max_requests` を受け取り、ハードリミット 500 を超える値はクランプする。1 未満は 1 にクランプ |
...
| `request_interval` | 1.0 | リクエスト間隔（秒）。許容範囲: 0.5〜60 |
| `max_requests` | 500 | 操作あたりリクエスト上限。上限: 500 |
...
| `clamp_request_interval` | リクエスト間隔（秒） | 0.5〜60.0 | 範囲外の値をクランプし、変更時は警告ログを出力 |
