...
- 取得した値をログ・print・デバッグ出力・エラーメッセージに含めないこと。値以外の情報（サービス名・キー名・例外詳細等）は記載してよい
- 運用者が事前にシークレットを登録する。登録コマンド例:

```
uv run python -c "import keyring, getpass; keyring.set_password('<service>', '<key>', getpass.getpass('Value: '))"
```
